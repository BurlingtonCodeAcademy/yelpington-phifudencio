...
function map() {
  fetch("./api/restaurants.json") //fectch the restaurants.json with all the restaurants
    .then((places) => {
      return places.json();
    })
    .then((placesArray) => {
      ...
      placesArray.forEach((restaurant) => {
        //For each restaurant will build this description
        ...
        let cordinates = JSON.parse(restaurant.coords); //get the coordinates of each restaurant
        let marker = L.marker([cordinates[0], cordinates[1]]).addTo(mymap); //Marker for each restaurant listed
        markers[restaurant.id] = marker;
        //make each pop up for the restaurant
        marker.bindPopup(
          `<p class= "popup" style="margin: .5px; text-align: center;"> ${restaurant.name}</br>${restaurant.address}</p>`
        );
      });
      return placesArray;
    })
